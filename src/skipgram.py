...
class SkipGramNeg(nn.Module):
    ...
    def __init__(self, n_vocab: int, n_embed: int, noise_dist: torch.Tensor = None):
        ...
        self.in_embed: nn.Embedding = nn.Embedding(num_embeddings=n_vocab, embedding_dim=n_embed)
        self.out_embed: nn.Embedding = nn.Embedding(num_embeddings=n_vocab, embedding_dim=n_embed)
...
class NegativeSamplingLoss(nn.Module):
    ...
    def forward(self, input_vectors: torch.Tensor, output_vectors: torch.Tensor,
        ...
        out_loss = torch.log(torch.sigmoid(torch.bmm(output_vectors.unsqueeze(1), input_vectors.unsqueeze(2)).squeeze())).sum()
